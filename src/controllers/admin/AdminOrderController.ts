...
class AdminOrderController {
		...
		try {
			const [ordersResult, statusCountResult] = await Promise.all([
				getRepository(Order).findAndCount({
					...
					where: status
						? baseWhere.map((condition) => ({ ...condition, status }))
						: baseWhere,
				}),
	...
	static services = async (req: Request, res: Response): Promise<Response> => {
		...
		const servicesToDelete = order.orderServices.filter(
			(orderService) =>
				!services.find((s) => s.serviceId == orderService.serviceId),
		);
		...
		for (const service of services) {
			...
			let orderService = order.orderServices.find(
			...
			if (service.colors && service.colors.length > 0) {
				orderService.colors = await getRepository(Color).findBy({
					id: In(service.colors),
				});
			}
		...
		try {
			const totalSections = newOrderServices.reduce(
				(acc, curr) => acc + curr.service.section,
				0,
			);
	...
	static payment = async (req: Request, res: Response): Promise<Response> => {
		const { id } = req.params;
		const { shouldUseWallet, method, finalPrice, refId, description } =
			req.body;
	...
	static sendPortal = async (
		...
		try {
			...
			if (!payment.id) {
				payment.randomCode = await getUniqueSlug(
					getRepository(Payment),
					generateCode(8, dataTypes.number),
					"randomCode",
				);
			}
	...
	static getRelatedWorkers = async (
		req: Request,
		res: Response,
	): Promise<Response> => {
		const { id } = req.params;
		...
		const busyWorkerIds = await getRepository(WorkerOffs).find({
			select: ["userId"],
			where: [
				{
					date: date,
		...
		if (id) {
			...
			for (const suggestedWorker of suggestedWorkers) {
				const closeOrder: Order = suggestedWorker.jobs.find(
					(e) =>
						e.date == date &&
						e.fromTime < fromTime &&
						e.fromTime >= fromTime - 2,
				);
				if (closeOrder) {
					...
					response = await axios.get(
						`https://api.neshan.org/v1/distance-matrix/no-traffic`,
						{
							params: {
								type: "car",
								origins:
									closeOrder.address.latitude +
									"," +
									closeOrder.address.longitude,
								destinations:
					...
					const suggestedWorkerIndex = suggestedWorkers.findIndex(
						(e) => e.id == suggestedWorker.id,
					);
	...
	private static assignOrder = async (
		order: Order,
		workerId: number,
		...
		try {
			...
			if (shouldSendSms) {
				...
				const servicesTitles =
					order.orderServices?.map((os) => os.service.title).join("-") || "";
