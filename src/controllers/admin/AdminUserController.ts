...
class AdminUserController {
	...
	static index = async (req: Request, res: Response): Promise<Response> => {
		...
		if (Array.isArray(relations)) {
			relations.forEach((relationName) => {
				const relation = getRepository(User).metadata.relations.find(
		...
		const options: FindManyOptions = {
			...
			where: role
				? baseWhere.map((condition) => ({ ...condition, role }))
				: baseWhere,
		};
	...
	static basic = async (req: Request, res: Response): Promise<Response> => {
		...
		if (role == roles.WORKER) {
			if (services) {
				...
				const serviceDescendants = await Promise.all(
					services.map(async (serviceId: number) => {
						const service = await getRepository(Service).findOneBy({
							id: serviceId,
						});
						if (!service) return [];
						const descendants = await getTreeRepository(
							Service,
						).findDescendants(service, { depth: 5 });
			...
			if (districts) {
				user.districts = await getRepository(District).findBy({
					id: In(districts),
				});
			}
	...
	static medias = async (req: Request, res: Response): Promise<Response> => {
		...
		if ((req as any).files[0]) {
			user.profileId = await media.create(
				req,
				(req as any).files[0],
				user.name + "-" + user.lastName,
				"/public/uploads/profilePic/",
			);
		}
