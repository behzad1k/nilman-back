...
class OrderController {
		...
		try {
			if (user.role === "WORKER") {
				orders = await this.orders().find({
					...
					relations: [
						"orderServices.colors",
						"orderServices.media",
						"service",
						"address",
						"worker.profilePic",
						"orderServices.addOns.addOn",
					],
				});
			} else {
				orders = await this.orders().find({
	...
	static workers = async (req: Request, res: Response): Promise<Response> => {
		...
		const nearest = await this.findFreeWorker(
			workers,
			parseInt(section?.toString()),
		);
		return res.status(200).send({
			code: 200,
			data: {
	...
	static deleteCartService = async (
		req: Request,
		res: Response,
	): Promise<Response> => {
		const { id } = req.params;
	...
	static findFreeWorker = async (workers: User[], section: number) => {
		...
		let nearestDate = moment(),
			nearestTime = Number(nearestDate.format("HH")),
			nearestWorker;
		for (const worker of workers) {
			const userWorkOffs = worker.workerOffs.filter(
				(value) =>
					moment(parseInt(value.date)).format("jDD") === nowDate.format("jDD"),
			);
			if (userWorkOffs.length == 0) {
				return {
					worker: worker.id,
					date:
						nowDate.format("jYYYY/jMM/jDD") +
						" " +
						nowHour +
						"-" +
						(nowHour + section),
			...
			for (let nowHourTmp = 22 - section; nowHourTmp >= nowHour; nowHourTmp--) {
				for (const userWorkOff of userWorkOffs) {
					if (
						userWorkOff.fromTime <= nowHourTmp &&
						userWorkOff.toTime > nowHourTmp
					) {
					...
					if (
						userWorkOff.fromTime <= nowHourTmp - section &&
						userWorkOff.toTime > nowHourTmp - section
					) {
						nowHourTmp = userWorkOff.fromTime;
						continue;
					}
		...
		return {
			...
			date:
				nearestDate.format("jYYYY/jMM/jDD") +
				" " +
				nearestTime +
				"-" +
				(nearestTime + section),
	...
	static create = async (req: Request, res: Response): Promise<Response> => {
		...
		if (discount) {
			try {
				discountObj = await this.discounts().findOneOrFail({
					where: { code: discount },
				});
			} catch (error) {
				res.status(400).send({
			...
			if (
				(discountObj.forUserId && discountObj.forUserId !== userId) ||
			...
			if (
				moment().unix() >
				moment(discountObj.createdAt)
					.add(discountObj.expirationDay, "day")
					.unix()
			) {
				return res.status(400).send({
					code: 1013,
			...
			if (
				await getRepository(Order).findOne({
					where: { userId: Number(userId), discountId: discountObj.id },
				})
			) {
				return res.status(400).send({
					code: 1014,
			...
			await this.discounts().update(
				{ id: discountObj.id },
				{ timesUsed: discountObj.timesUsed + 1 },
			);
		}
		...
		if (discountObj && (discountObj.amount || discountObj.percent)) {
			const discountAmount = discountObj.percent
				? (totalPrice * discountObj.percent) / 100
				: discountObj.amount;
		...
		try {
			...
			await Promise.all(
				attributeObjs.map(async (attr) => {
					...
					orderService.service = await getRepository(Service).findOneBy({
						id: attr.id,
					});
					orderService.price =
						attr.price *
					...
					if (attributes[attr.id]?.colors?.length > 0) {
						orderService.colors = await getRepository(Color).find({
							where: { slug: In(attributes[attr.id]?.colors) },
						});
					}
					...
					if (attributes[attr.id]?.addOns) {
						for (const [key, value] of Object.entries(
							...
							orderServiceAddOn.price =
								orderServiceAddOn.singlePrice * orderServiceAddOn.count;
							...
							addOnOrderService.price =
								addOnObj.price *
								(isUrgent ? 1.5 : 1) *
								Number((value as any)?.count);
							addOnOrderService.singlePrice =
								addOnObj.price * (isUrgent ? 1.5 : 1);
							// if (attributes[attr.id]?.colors?.length > 0) {
	...
	static medias = async (req: Request, res: Response): Promise<Response> => {
		...
		for (const media of (req as any).files) {
			const orderService = await getRepository(OrderService).findOne({
				where: {
					service: {
						id: media.fieldname.substring(6, media.fieldname.length - 1),
					},
					order: {
						id: Number(id),
			...
			if (orderService) {
				orderService.mediaId = await Media.create(
					req,
					media,
					orderService.service.title +
						"-" +
						orderService.order.user.phoneNumber,
	...
	static applyDiscount = async (
		...
		try {
			...
			orderObj = discountObj.serviceId
				? user.orders.find(
						(e) => e.inCart && e.serviceId == discountObj.serviceId,
					)
				: user.orders[0];
		} catch (error) {
			res.status(400).send({
	...
	static pay = async (req: Request, res: Response): Promise<Response> => {
		...
		for (const order of orders) {
			if (
				moment(moment().subtract(1, "hour")).diff(
					order.createdAt,
					"hour",
					true,
				) > 1
		...
		let finalPrice: any = orders.reduce<number>(
		...
		try {
			...
			payment.randomCode = await getUniqueSlug(
			...
			if (isCredit) {
				creditUsed =
					user.walletBalance > finalPrice ? finalPrice : user.walletBalance;
		...
		if (isCredit && finalPrice == creditUsed) {
			...
			try {
				...
				const sepReq = await axiosInstance.post(
			...
			const zarinpal = ZarinPalCheckout.create(
				"f04f4d8f-9b8c-4c9b-b4de-44a1687d4855",
				false,
			);
			const zarinpalResult = await zarinpal
				.PaymentRequest({
	...
	static paymentVerify = async (
		req: Request,
		res: Response,
	): Promise<Response> => {
		const { authority, status, refNum, terminalId, tranId } = req.body;
		...
		let decryptedValue = ",,,,,,";
		if (tranId) {
			decryptedValue = decryptVectors(
				"jlQxyd+MtWbB4iRUAOVsrUm45zz/vFGzgn1atomY5lw=",
				"ztPE0f1sZFtNn3C+7yEzB96+9bcr3/CqXpf3RgOTc9I=",
				authority,
			);
		}
		try {
		...
		try {
			if (payment.method == "zarinpal") {
				...
				const zarinpalRes = await zarinpal
					.PaymentVerification({
		...
		try {
			for (const order of orders) {
				...
				if (order.workerId) {
					const worker = await getRepository(User).findOneBy({
						id: order.workerId,
					});
					...
					sms.orderAssignWorker(
						...
						order.orderServices
							?.reduce((acc, cur) => acc + "-" + cur.service.title, "")
							.toString(),
				...
				const triggeredOrderService = order.orderServices.find(
					(e) => e.service?.triggerPackage?.id,
				);
				if (triggeredOrderService) {
					...
					for (const newService of packageUsed.services) {
						const ancestors =
							await getTreeRepository(Service).findAncestors(newService);
